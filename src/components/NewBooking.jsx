...
export default function NewBooking({ setOpen, setAlert }) {
  const formRef = useRef(null);
  const auth = useAuth();
  const customers = useFetch(endPoints.customers.getAllCustomer);
  ...
  const enableNext = () => {
    const customerId = parseInt(new FormData(formRef.current).get('customer'));
    if (customerId && (selectedService != 0 || selectedPromo != 0)) {
              ...
              <div className="col-span-8 sm:col-span-8">
                ...
                >
                  ...
                  {customers?.map((customer) => (
                    <option key={customer.id} value={customer.id} disabled={customer.unavailable} className="text-xs leading-5 text-gray-600 py-0 pl-4 whitespace-nowrap">
                      {customer.user.email} ({customer.firstName} {customer.lastName})
                    </option>
                  ))}
                </select>
              </div>
