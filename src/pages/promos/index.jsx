...
export default function Promos() {
  ...
  return (
    <ul className="divide-y divide-gray-200">
      ...
      {promos.map((promo) => (
        <li key={`service-${promo.id}`} className="py-2 px-2">
          <div as="div">
            <>
              <div as="div" className="flex justify-between gap-x-6">
                ...
                <div className="flex flex-col items-end">
                  <div className="flex flex-raw gap-x-6 px-2">
                    <div>
                      <button type="button" className="flex gap-x-2" onClick={() => handleEdit(promo.id)}>
                    ...
                    <div>
                      <button type="button" className="flex gap-x-2" onClick={() => handleView(promo.id)}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-gray-600">
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
              ...
              <div className="flex justify-between">
                <div className="flex gap-x-4">
                  <div className="min-w-0 flex-auto">
                    ...
                    <p className="mt-1 truncate text-xs leading-3 text-gray-500"> {promo.services?.map((service) => `- ${service.name} `)}</p>
                  </div>
                </div>
              </div>
            </>
